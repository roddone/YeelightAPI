...
namespace YeelightAPI
{
  ...
  public partial class Device : IDisposable
  {
    ...
    /// </summary>
    private readonly Dictionary<int, ICommandResultHandler> _currentCommandResults =
      new Dictionary<int, ICommandResultHandler>();
    ...
    /// </summary>
    public bool IsConnected { get { return _tcpClient != null && _tcpClient.IsConnected(); } }
    ...
    internal Device(
      ...
      List<METHODS> supportedOperations)
    {
      Hostname = hostname;
      Port = port;
      Id = id;
    ...
    /// Name of the device
    /// </summary>
    public string Name { get { return this[PROPERTIES.name] as string; } set { this[PROPERTIES.name] = value; } }
    ...
    internal void ExecuteCommand(METHODS method, int id, List<object> parameters = null)
    {
      ...
      byte[]
        sentData = Encoding.ASCII.GetBytes(
          data + Constants
    ...
    /// <param name="parameters"></param>
    /// <returns></returns>
    internal async Task<CommandResult<T>> ExecuteCommandWithResponse<T>(
      METHODS method,
      int id,
    ...
    /// <returns></returns>
    private async Task<CommandResult<T>> UnsafeExecuteCommandWithResponse<T>(
      METHODS method,
      int id = 0,
      List<object> parameters = null)
    {
                  ...
                  if (_tcpClient.Client.Available > 0)
                  {
                    ...
                    try
                    {
                      ...
                      if (!string.IsNullOrEmpty(datas))
                      {
                        ...
                        foreach (string entry in datas.Split(
                          new string[] {Constants.LineSeparator},
                          StringSplitOptions.RemoveEmptyEntries))
                        {
                          ...
                          if (commandResult != null && commandResult.Id != 0)
                          {
                            ...
                            {
                              commandResult = (CommandResult) JsonConvert.DeserializeObject(
                                entry,
                                commandResultHandler.ResultType,
                          ...
                          else
                          {
                            NotificationResult notificationResult =
                              JsonConvert.DeserializeObject<NotificationResult>(
                                entry,
                                Constants.DeviceSerializerSettings);
